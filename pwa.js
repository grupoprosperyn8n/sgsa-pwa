...
function renderConversations(){
  ...
  c.innerHTML=sorted.map(cv=>{
    ...
    const sel=_batchSelected.includes(gid);
    return`<div class="group-card ${cv.pinned?"pinned ":""}${selectedConversation?.group_id===gid?"selected":""}${_batchMode?" batch-mode":""}${_batchMode&&sel?" batch-selected":""}" data-gid="${gid}" onclick="_cardClick(${gid},event)">
