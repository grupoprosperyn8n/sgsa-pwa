...
function renderMsgList(c,e,gid,msgs){
  ...
  for(const m of sorted){
    ...
    if(m.tipo==="imagen"&&m.adjunto_url)body=`<div class="msg-attachment"><div class="att-preview"><img src="${m.adjunto_url}" loading="lazy"><button class="att-dl" data-url="${m.adjunto_url}" data-name="${esc(m.adjunto_nombre||"imagen")}" title="Descargar"><span class="material-symbols-outlined">download</span></button></div></div>`;
    else if((m.tipo==="video"||m.tipo==="audio")&&m.adjunto_url)body=`<div class="msg-attachment"><div class="att-preview">${m.tipo==="audio"?`<audio controls src="${m.adjunto_url}"></audio>`:`<video controls src="${m.adjunto_url}" style="max-width:100%;max-height:300px"></video>`}<button class="att-dl" data-url="${m.adjunto_url}" data-name="${esc(m.adjunto_nombre||m.tipo)}" title="Descargar"><span class="material-symbols-outlined">download</span></button></div></div>`;
