...
function renderConversations(){
  ...
  c.innerHTML=sorted.map(cv=>{
    ...
    <button class="pin-btn ${cv.pinned?"pinned":""}" data-gid="${gid}" title="${cv.pinned?"Desfijar":"Fijar"}" onclick="event.stopPropagation();_pinChat(${gid},this)"><span class="material-symbols-outlined">push_pin</span></button>
    <button class="delete-chat-btn" data-gid="${gid}" title="Archivar chat" onclick="event.stopPropagation();_archiveChat(${gid})"><span class="material-symbols-outlined">archive</span></button>
  </div>`}).join("");
}
