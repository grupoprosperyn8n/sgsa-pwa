...
function renderAlerts(){
  ...
  // D-open buttons
  c.querySelectorAll(".d-open").forEach(b=>b.addEventListener("click",e=>{e.stopPropagation();const idx=+b.dataset.idx;showAlertDetail(alerts[idx])}));
  // Delegated card + actions
  if(!c._del){c._del=true;c.addEventListener("click",async e=>{const btn=e.target.closest(".act-btn");if(btn){e.stopPropagation();const card=btn.closest(".alert-card"),id=card?.dataset.id;if(!id)return;if(btn.classList.contains("ack"))await doAck(id);else if(btn.classList.contains("progreso"))await doStatus(id,"EN_PROGRESO");else if(btn.classList.contains("confirmar"))await doStatus(id,"TURNO_CONFIRMADO");else if(btn.classList.contains("concluido"))await doStatus(id,"CONCLUIDA");else if(btn.classList.contains("anular"))await doStatus(id,"ANULADA");loadAlerts();alertsSound&&Sound.alert();return}if(e.target.closest(".d-link,.d-open"))return;const card=e.target.closest(".alert-card");if(card)card.classList.toggle("expanded")})}
}
...
async function loadMessages(gid){const d=await G("/api/chat/mensajes/"+gid),c=document.getElementById("messageList"),e=document.getElementById("messageEmpty");if(!d?.ok){c.innerHTML='<div class="empty-state"><p>Error al cargar mensajes</p></div>';return}const msgs=d.mensajes||[];if(!msgs.length){e.style.display="flex";c.innerHTML="";return}e.style.display="none";const myId=currentUser?.airtable_id||currentUser?.id||"";c.innerHTML=msgs.reverse().map(m=>{const isMine=m.remitente_id===myId;let body=`<div class="msg-text">${esc(m.texto||"")}</div>`;if(m.tipo_mensaje==="imagen"&&m.archivo_url)body=`<div class="msg-attachment"><img src="${m.archivo_url}" loading="lazy"></div>`;else if((m.tipo_mensaje==="video"||m.tipo_mensaje==="audio")&&m.archivo_url)body=`<div class="msg-attachment">${m.tipo_mensaje==="audio"?`<audio controls src="${m.archivo_url}"></audio>`:`<video controls src="${m.archivo_url}" style="max-width:100%;max-height:300px"></video>`}</div>`;else if(m.archivo_url)body=`<div class="msg-attachment"><a class="file-link" href="${m.archivo_url}" target="_blank"><span class="material-symbols-outlined" style="font-size:16px">attach_file</span>${esc(m.archivo_nombre||"Archivo")}</a></div>`;return`<div class="message ${isMine?"mine":"theirs"}"><div class="sender-name" style="font-size:11px;color:var(--fg3);margin-bottom:2px">${!isMine?esc(m.remitente_nombre||""):""}</div>${body}<div class="msg-time">${timeAgo(m.created_at)}</div></div>`}).join("");c.scrollTop=c.scrollHeight}

async function sendMessage(){const input=document.getElementById("chatInput"),text=input.value.trim();if(!text||!selectedConversation)return;input.value="";const payload={grupo_id:selectedConversation.group_id,empleado_id:currentUser?.airtable_id,contenido:text};const d=await P("/api/chat/send",payload);if(d?.ok){stats.msgsSent++;loadMessages(selectedConversation.group_id);refreshConversations()}else{enqueueOffline(payload);toast("Mensaje encolado — se enviará al reconectar","error")}}
